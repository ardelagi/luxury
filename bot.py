...
class LuxuryBotCog(commands.Cog):
    def __init__(self, bot: commands.Bot):
        ...
        # Dihapus dari sini agar bisa dipanggil manual saat on_ready
    ...
    # (Fungsi _parse_data_file, _fetch_url, fetch_data, get_latest_commit, _ask_gemini tetap sama)
    ...
    async def _fetch_url(self, url: str) -> Optional[Dict[str, Any] | str]:
        try:
            async with self.session.get(url, timeout=10) as res:
                ...
                else: logger.warning(f"Gagal ambil data dari {url}. Status: {res.status}")
        except Exception as e:
            await log_error(f"Error fetching URL {url}: {e}")
    ...
    async def fetch_data(self) -> bool:
        ...
        if isinstance(content, str):
            ...
            if parsed["products"] or parsed["faq"]:
                ...
                return True
            else: logger.warning("⚠️ Data berhasil diunduh, namun tidak ada produk/FAQ valid yang ditemukan setelah parsing.")
        return False
    ...
    async def _ask_gemini(self, prompt: str) -> str:
        if not self.model: return "Maaf, fitur AI saat ini tidak tersedia."
        try:
            loop = asyncio.get_event_loop()
            response = await loop.run_in_executor(None, lambda: self.model.generate_content(prompt))
            return response.text.strip()
        except Exception as e:
            await log_error(f"Error saat memanggil Gemini AI: {e}")
            logger.error(f"Error AI response: {e}")
            return "Maaf, terjadi kesalahan saat memproses pertanyaan Anda. Silakan coba lagi nanti."
    
    # --------------------------------------------------------------------------
    # Background Task & Listeners (BAGIAN YANG DIPERBAIKI)
    # --------------------------------------------------------------------------
    ...
    async def auto_update_data(self):
        ...
        # Logika baru: Jika cache masih kosong, paksa fetch data.
        ...
        if force_fetch or (new_commit and new_commit != self.last_commit_hash):
            ...
            if await self.fetch_data():
                ...
                # Kirim notifikasi hanya jika bukan pengambilan data pertama kali
    ...
    async def on_ready(self):
        ...
        # Panggil loop sekali secara manual saat startup untuk memuat data awal.
        # Ini memberikan kepastian data dimuat saat bot nyala.
        await self.auto_update_data()
        
        # Baru jalankan loop terjadwal setelah pemuatan pertama selesai.
    ...
    # (on_command_error dan semua command lain tetap sama persis)
    @commands.Cog.listener()
    async def on_command_error(self, ctx: commands.Context, error: commands.CommandError):
        if isinstance(error, commands.CommandNotFound): return
            ...
            embed = discord.Embed(title="❌ Terjadi Kesalahan", description="Sesuatu yang tidak terduga terjadi. Tim kami telah diberitahu.", color=discord.Color.red())
            await ctx.reply(embed=embed, ephemeral=True)

    # ... [ SEMUA COMMAND SEPERTI !tanya, !faq, !stock, dll. DITEMPATKAN DI SINI TANPA PERUBAHAN ] ...
    ...
    async def tanya(self, ctx: commands.Context, *, query: str):
        ...
        async with ctx.typing():
            ...
            if not related_products:
                ...
                for p in related_products: context += f"- **{p['name']}** (Kategori: {p['category']}) | Harga: Rp {p['price']} | Stok: {p['stock']}\n"
            ...
            embed = discord.Embed(title="💬 Jawaban Luxury VIP", description=answer, color=discord.Color.gold() if related_products else discord.Color.orange(), timestamp=datetime.now())
            if related_products and len(related_products) <= 3:
                for p in related_products:
                    stock_emoji = "✅" if p['stock'].lower() not in ["habis", "0"] else "❌"
                    embed.add_field(name=f"{stock_emoji} {p['name']}", value=f"🏷️ {p['category']}\n💰 Rp {p['price']}\n📊 Stok: {p['stock']}", inline=True)
    ...
    async def faq(self, ctx: commands.Context):
        if not self.data_cache["faq"]:
            ...
            return
        embed = discord.Embed(title="❓ FAQ - Pertanyaan yang Sering Ditanyakan", description="Pilih pertanyaan dari menu di bawah untuk melihat jawabannya:", color=discord.Color.blue(), timestamp=datetime.now())
        class FAQSelect(Select):
            def __init__(self, faq_data: List[Dict[str, str]]):
                self.faq_data = faq_data
                options = [discord.SelectOption(label=f"Q{idx+1}: {item['question'][:80]}", value=str(idx), emoji="❓") for idx, item in enumerate(faq_data[:25])]
        ...
        view = View(timeout=180); view.add_item(FAQSelect(self.data_cache["faq"])); await ctx.reply(embed=embed, view=view)
    ...
    async def stock(self, ctx: commands.Context, *, category: Optional[str] = None):
        if not self.data_cache["products"]:
            ...
            return
        if not category:
            embed = discord.Embed(title="🏷️ Kategori Produk", description="Gunakan `!stock <kategori>` untuk melihat produk.\nContoh: `!stock VIP Gold`", color=discord.Color.blue(), timestamp=datetime.now())
        ...
        for item in products[:25]:
            ...
            embed.add_field(name=f"{stock_emoji} {item['name']}", value=f"💰 **Harga:** Rp {item['price']}\n📊 **Stok:** {item['stock']}", inline=False)
        await ctx.reply(embed=embed)
    @commands.command(name="help")
    async def help_command(self, ctx: commands.Context):
        embed = discord.Embed(title="💎 Luxury VIP Assistant", description="Selamat datang! Saya adalah bot asisten untuk Luxury VIP.\n\n**📋 Command Tersedia:**\n• `!help` - Menampilkan menu bantuan ini\n• `!faq` - Pertanyaan yang sering ditanyakan\n• `!stock [kategori]` - Melihat produk berdasarkan kategori\n• `!tanya <pertanyaan>` - Bertanya tentang produk (AI)\n• `!ping` - Cek latensi bot\n• `!status` - Cek status sistem bot", color=discord.Color.gold(), timestamp=datetime.now())
        ...
        class HelpSelect(Select):
            def __init__(self, cog_instance):
                ...
                options = [discord.SelectOption(label="❓ FAQ", description="Cara kerja command !faq", emoji="❓"), discord.SelectOption(label="📦 Stock", description="Cara kerja command !stock", emoji="📦"), discord.SelectOption(label="💬 Tanya", description="Cara bertanya dengan AI", emoji="💬"), discord.SelectOption(label="🤖 Tentang Bot", description="Info sistem dan auto update", emoji="🤖")]
    ...
    async def status(self, ctx: commands.Context):
        ...
        embed.add_field(name="📡 Latency", value=f"{round(self.bot.latency * 1000)}ms", inline=True); embed.add_field(name="🏢 Servers", value=f"{len(self.bot.guilds)}", inline=True); embed.add_field(name="🔄 Auto-Update", value="🟢 Aktif" if self.auto_update_data.is_running() else "🔴 Nonaktif", inline=True); embed.add_field(name="📦 Total Produk", value=f"{len(self.data_cache['products'])}", inline=True); embed.add_field(name="🏷️ Kategori", value=f"{len(self.product_categories)}", inline=True); embed.add_field(name="❓ FAQ", value=f"{len(self.data_cache['faq'])}", inline=True); embed.add_field(name="✅ Stok Tersedia", value=f"{tersedia}", inline=True); embed.add_field(name="💾 Commit", value=f"`{self.last_commit_hash[:7]}`" if self.last_commit_hash else 'N/A', inline=True)
        await ctx.reply(embed=embed)
